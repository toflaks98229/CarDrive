...
- **GhostSpawner**: **차의 부품**입니다. 시동이 켜져 있을 때만 차에 달라붙는 귀신을 스폰하며, 날씨와 시간대에 따라 간격이 좁아집니다. (폭우 ×1.9 × 밤 ×1.5)
- **NightGhostSpawner**: **밤이 되면 세계에 나오는** 배회 귀신입니다. 차와 무관하게 돌고, 플레이어가 운전 중이든 걷고 있든 그 주위 30~60 m 고리에 놓습니다. **보는 쪽의 반대 부채꼴**에서 태어나므로 눈앞에서 생기지 않고, 날이 밝으면 죽이지 않고 조용히 거둡니다. 동시 마릿수에 상한이 있습니다.
- **EnemyController**: Rigidbody로 추적하는 일반 적. **도보면 사람을, 주행이면 그 차를** 노립니다 — 예전에는 차만 골라, 차에서 내리는 순간 빈 차 주위를 맴돌았습니다.
