...
namespace CarDrive.EditorTools
{
    ...
    public class WorldTerrainBaker : EditorWindow
    {
        ...
        [Tooltip("길 양옆으로 몇 칸을 더 깔지. 0이면 길만 따라가는 좁은 회랑이 됩니다.")]
        private int roadCorridorTiles = 2;
        ...
        private void OnGUI()
        {
            ...
            roadCorridorTiles = EditorGUILayout.IntSlider("길 옆 타일", roadCorridorTiles, 0, 4);
        ...
        private void Bake()
        {
            ...
            // <b>지우기 전에</b> 지금 깔려 있는 옷을 알아 둡니다.
            // 지우고 나면 어떤 지면 머티리얼과 풀을 쓰고 있었는지 알 길이 없습니다.
            List<string> dressReport = new List<string>();
            Terrain[] before = Object.FindObjectsByType<Terrain>(FindObjectsInactive.Include);
            Material dressMaterial = TerrainDressing.ResolveMaterial(before, dressReport);
            GameObject grassPrefab = TerrainDressing.ResolveGrassPrefab(before, dressReport);

            ClearBaked(false);

            // 높이맵을 처음부터 다시 만들므로 올려 둔 산은 이 시점에 사라집니다.
            // 그 사실을 알려 두지 않으면 다음 '산 만들기'가 없는 산을 걷어 내며 땅을 파냅니다.
            ProcGen.MountainTool.ForgetStamp();
            ...
            try
            {
                for (int i = 0; i < coords.Count; i++)
                {
                    ...
                    Terrain terrain = BakeTile(coords[i], center, roads, layers, dressMaterial, root);
            ...
            // 지면 머티리얼 · 지면 레이어 · 화면 오차 · 풀을 다시 입힙니다.
            //
            // 굽기는 TerrainData 를 새로 만들기 때문에 이 넷이 전부 사라집니다.
            // 예전에는 굽고 나서 룩 도구를 따로 돌려야 했고, 잊으면 풀이 없고
            // 소품이 지면을 뚫는 상태로 남았습니다. 이제 굽기가 여기서 마무리합니다.
            Terrain[] baked = new Terrain[built.Count];
            built.Values.CopyTo(baked, 0);
            TerrainDressing.Apply(baked, dressMaterial, grassPrefab, dressReport);
            ...
            lastReport =
                ...
                "씬을 저장해야 유지됩니다.\n" +
                "\n" +
                "높이맵을 새로 만들었으므로 다음 셋은 다시 해야 합니다.\n" +
                "  0. 산 만들기 → 2. 나무 · 바위 흩뿌리기 → 3. 마을 세우기";
        ...
        private List<Vector2Int> CollectTileCoords(Vector3 center)
        {
            ...
            //
            // 길 옆에 깐 만큼은 마을 주위에도 깔아야 합니다. 그러지 않으면 마을에 서서
            // 길이 없는 쪽을 볼 때만 땅이 가까이서 끊깁니다. 길 옆 회랑은 (칸수+0.5)칸까지
            // 뻗으므로 마을에는 그보다 한 칸 넉넉히 잡습니다.
            int pad = Mathf.Max(villagePadTiles, roadCorridorTiles + 1);
            pad = Mathf.Max(pad, Mathf.CeilToInt(villageRadius / tileSize));
            ...
            if (streamer.routes != null)
            {
                for (int r = 0; r < streamer.routes.Count; r++)
                {
                    ...
                    // <= 인 것은 길 <b>끝 칸까지</b> 회랑을 붙이기 위해서입니다.
                    // 마지막 칸만 따로 넣으면 길 끝이 한 칸짜리 돌기로 삐져나옵니다.
                    for (int i = 0; i <= route.tileCount; i++)
                    {
                        ...
                        // 길 양옆으로 회랑을 넓힙니다.
                        //
                        // 반 칸(0.5)은 길이 타일 경계를 비스듬히 지나갈 때 옆 칸이 비지 않게 하는
                        // <b>최소치</b>입니다. 그 위로 한 칸씩 얹어 길 옆에 지형이 설 자리를 만듭니다.
                        //
                        // 반 칸씩 밀어 두는 덕에 길이 타일 경계 위에 놓여도 <b>좌우가 대칭</b>이 됩니다.
                        // 정수 칸으로 밀면 길이 놓인 칸이 한쪽으로 쏠려 한 칸만큼 어긋납니다.
                        Vector3 side = Vector3.Cross(dir, Vector3.up).normalized;
                        for (int k = 0; k <= roadCorridorTiles; k++)
                        {
                            float reach = tileSize * (k + 0.5f);
                            set.Add(ToCoord(cursor + side * reach));
                            set.Add(ToCoord(cursor - side * reach));
                        }
