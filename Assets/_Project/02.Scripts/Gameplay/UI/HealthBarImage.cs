...
namespace CarDrive.UI
{
    ...
    public class HealthBarImage : MonoBehaviour
    {
        ...
        /// <summary>
        /// 숫자를 만들 때 재사용하는 버퍼입니다. 매번 새 문자열을 이어 붙이지 않기 위한 것입니다.
        /// </summary>
        private readonly StringBuilder builder = new StringBuilder(16);

        /// <summary>마지막으로 그린 현재 체력입니다. 그대로면 다시 그리지 않습니다.</summary>
        private int lastDrawnCurrent = int.MinValue;

        /// <summary>마지막으로 그린 최대 체력입니다.</summary>
        private int lastDrawnMax = int.MinValue;
        ...
        /// <summary>
        /// 체력 숫자를 갱신합니다. <b>값이 바뀌었을 때만 씁니다.</b>
        ///
        /// 예전에는 매 프레임 <c>int + " / " + int</c>를 새로 만들어 넣었습니다.
        /// 체력은 대부분의 시간 동안 그대로인데도 프레임마다 문자열이 쓰레기로 쌓였습니다.
        /// 퍼센트와 달리 체력은 값의 범위가 정해져 있지 않아 미리 만들어 둘 수 없으므로,
        /// <see cref="TextHealthBar"/>가 쓰는 방식대로 버퍼를 재사용합니다.
        /// </summary>
        /// <param name="normalized">0에서 1 사이의 체력 비율</param>
        private void UpdateValueText(float normalized)
        {
            if (valueText == null) return;

            int current = Mathf.CeilToInt(normalized * source.maxHealth);
            int max = Mathf.RoundToInt(source.maxHealth);

            if (current == lastDrawnCurrent && max == lastDrawnMax) return;

            lastDrawnCurrent = current;
            lastDrawnMax = max;

            builder.Length = 0;
            builder.Append(current).Append(" / ").Append(max);
            valueText.SetText(builder);
        }
    }
}
