...
namespace CarDrive.Gameplay
{
    ...
    /// </summary>
    public class PlayerCameraShake : MonoBehaviour, IImpactShakable
    {
        ...
        ///
        /// <b>방향은 쓰지 않습니다.</b> 시야 흔들림은 노이즈로 만들어서 어느 쪽에서 맞았는지가
        /// 그림에 드러나지 않습니다. 그래도 인자를 받는 이유는, 부딪히는 쪽이 흔들 대상마다
        /// 다른 메서드를 골라 부르지 않아도 되게 하기 위해서입니다.
        /// </summary>
        /// <param name="worldDirection">충격 방향(월드). 여기서는 무시합니다.</param>
        /// <param name="scale">세기 배율</param>
        public void TriggerImpactShake(Vector3 worldDirection, float scale)
        {
            TriggerImpactShake(scale);
        }
