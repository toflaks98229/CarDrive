...
namespace CarDrive.Gameplay
{
    ...
    public class PlayerModeController : MonoBehaviour
    {
        ...
        /// 플레이어가 지금 '있는' 자리입니다.
        ///
        /// 주행 중이면 차량, 도보면 도보 리그입니다. 그래서 바닥에 떨어진 것을
        /// <b>차를 타고 지나가면서도 주울 수 있습니다.</b> (CurrencyPickup 참고)
        /// 카메라를 쓰지 않는 이유가 있습니다. 카메라는 상태에 따라 부모가 바뀌고
        /// 두리번거리면 위치까지 흔들려서, 거리 판정 기준으로는 불안정합니다.
        /// </summary>
        public Transform PickupAnchor
        {
            get
            {
                if (Mode == PlayerMode.Driving && vehicle != null) return vehicle.transform;
                if (footRig != null) return footRig.transform;
                return transform;
            }
        }
        ...
        public bool CanExitVehicle()
        {
            ...
            return vehicle.controller.CurrentSpeed <= maxExitSpeed;
        }
        ...
        private bool ValidateReferences()
        {
            ...
            if (vehicle == null)
            {
                // 차량은 이미 자기 등록부(Vehicle.All)를 갖고 있습니다. 씬을 뒤질 이유가 없습니다.
                if (Vehicle.Current != null) vehicle = Vehicle.Current;
                else if (Vehicle.All.Count > 0) vehicle = Vehicle.All[0];
            }
            if (vehicle == null)
            {
