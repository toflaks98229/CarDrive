...
namespace CarDrive.Systems
{
    ...
    public class SkyController : MonoBehaviour
    {
        ...
        /// <summary>
        /// 빛이 <b>내려오는 각도</b>입니다(도). 90 이면 정수리에서 수직입니다.
        ///
        /// ⚠ <b>이 세계에는 하늘이 없습니다.</b> 머리 위를 덮은 것은 거대구조물이고,
        /// 하늘처럼 보이는 것은 그 천장을 구운 파노라마입니다. 그러니 빛의 근원은
        /// <b>해도 달도 아니라 천장</b>입니다 — 화면에 보이는 그 등불들입니다.
        ///
        /// 천장은 하루 종일 같은 자리에 있습니다. 그래서 <b>그림자의 방향은
        /// 시간이 가도 바뀌지 않습니다.</b>
        ///
        /// 90 도로 세우면 벽이 빛을 하나도 못 받아 통짜 실루엣이 됩니다.
        /// 조금 기울여 두면 벽에도 결이 남습니다.
        /// </summary>
        [Tooltip("빛이 내려오는 각도(도). 90 이면 수직. 세울수록 벽이 어두워집니다")]
        [Range(20f, 90f)]
        public float ceilingPitch = 65f;

        /// <summary>
        /// 빛이 내려오는 <b>방위</b>입니다(도). 그림자가 어느 쪽으로 눕는지를 정합니다.
        ///
        /// ⚠ <b>이 값은 시간이 가도 바뀌지 않습니다.</b> 천장이 근원이므로 그림자의
        /// 방향도 하루 종일 같습니다.
        /// </summary>
        [Tooltip("빛의 방위(도). 그림자가 눕는 쪽. 시간이 가도 바뀌지 않습니다")]
        [Range(0f, 360f)]
        public float ceilingYaw = 170f;

        /// <summary>
        /// 하루 동안 기울기가 <b>숨 쉬는 폭</b>입니다(도). 한낮에 이만큼 더 서고
        /// 한밤에 이만큼 더 눕습니다.
        ///
        /// <b>왜 0 이 아닌가.</b> 완전히 고정하면 하루가 <b>한 장의 그림</b>이 됩니다.
        /// 그림자의 <b>방향</b>은 그대로 두고 <b>길이만</b> 조금 움직이면, 시간이
        /// 흐른다는 것이 읽히면서도 아침 해처럼 길게 눕지는 않습니다.
        ///
        /// 0 으로 두면 완전히 고정됩니다.
        /// </summary>
        [Tooltip("하루 동안 기울기가 숨 쉬는 폭(도). 방향은 안 바뀌고 길이만 움직입니다")]
        [Range(0f, 20f)]
        public float ceilingSway = 6f;
        ...
        private void ApplySun(float daylight)
        {
            ...
            // ⚠ <b>해의 자리를 쓰지 않습니다.</b>
            //
            // <see cref="TimeSystem"/> 은 방향광을 하루에 한 바퀴 돌립니다
            // (<c>Euler(분/하루 × 360 − 90, 170, 0)</c>). 하늘이 있는 세계의 방법입니다.
            // <b>이 세계의 머리 위는 거대구조물</b>이고, 하늘처럼 보이는 것은 그 천장을
            // 구운 파노라마입니다. 빛의 근원은 그 <b>천장의 등불</b>이므로 자리가
            // 하루 종일 같아야 하고, 그러면 <b>그림자의 방향도 바뀌지 않습니다.</b>
            //
            // 실측(2026-09-11): 게임은 08시에 시작하는데 그때 해는 지평선 <b>30도
            // 위</b>였습니다. 대낮인데 그림자가 길게 눕던 것이 그래서였습니다 —
            // 구워 둔 그림자가 아니라 <b>아침 해</b>였습니다.
            //
            // 기울기만 조금 숨 쉽니다. 완전히 고정하면 하루가 한 장의 그림이 됩니다.
            sun.transform.rotation =
                Quaternion.Euler(CeilingPitch(daylight, ceilingPitch, ceilingSway),
                                 ceilingYaw, 0f);
        }

        /// <summary>
        /// 그 시각에 빛이 내려오는 <b>기울기</b>입니다(도).
        ///
        /// 방향(방위)은 건드리지 않습니다. 한낮에 조금 더 서고 한밤에 조금 더 누울
        /// 뿐이라 <b>그림자는 길이만</b> 움직입니다.
        /// </summary>
        /// <param name="daylight">0이면 한밤, 1이면 한낮</param>
        /// <param name="pitch">기준 기울기(도)</param>
        /// <param name="sway">숨 쉬는 폭(도)</param>
        /// <returns>10~90도로 잘라 낸 기울기</returns>
        public static float CeilingPitch(float daylight, float pitch, float sway)
        {
            float want = pitch + sway * (Mathf.Clamp01(daylight) * 2f - 1f);

            // ⚠ <b>90도를 넘기면 빛이 반대편에서 올라옵니다.</b> 10도 아래로 누우면
            // 아침 해가 되어, 고치려던 그림자가 그대로 돌아옵니다.
            return Mathf.Clamp(want, 10f, 90f);
        }
