...
namespace CarDrive.Systems
{
    ...
    public class NeedsDebugOverlay : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            IReadOnlyList<NeedSetting> all = needs.GetAllSettings();
            if (all == null) return;
        ...
        {
            if (GameInput.GetKeyDownRaw(KeyCode.Alpha1)) needs.Add(NeedType.Hunger, 0.1f);
            if (GameInput.GetKeyDownRaw(KeyCode.Alpha2)) needs.Add(NeedType.Thirst, 0.1f);
            if (GameInput.GetKeyDownRaw(KeyCode.Alpha3)) needs.Add(NeedType.Fatigue, 0.1f);
            if (GameInput.GetKeyDownRaw(KeyCode.Alpha4)) needs.Add(NeedType.Stress, 0.1f);
            if (GameInput.GetKeyDownRaw(KeyCode.Alpha5)) needs.Add(NeedType.Urine, 0.1f);
            if (GameInput.GetKeyDownRaw(KeyCode.Alpha6)) needs.Add(NeedType.Hygiene, 0.1f);
            if (GameInput.GetKeyDownRaw(KeyCode.Alpha0)) needs.ResetAll();
        }
    }
}
