...
namespace CarDrive.Systems
{
    ...
    /// 카메라 파클립을 직접 썼고, 실행 순서가 각각 0과 200이라
    ...
    public class ViewRangeScaler : MonoBehaviour
    {
        ...
        /// 랜드마크가 보이는 거리(m)입니다.
        ///
        /// ⚠ <b>안개가 닫히는 거리보다 멀어야 합니다.</b> 팔레트가 기둥에 준
        /// 배수가 5 이고 안개가 257 m 에서 닫히므로 1,285 m 에서 완전히 사라집니다.
        /// 그보다 멀게 두면, 평면 깊이가 언제나 방사 거리 이하라서 <b>잘리는 자리는
        /// 반드시 안개 안</b>입니다 — 파클립이 평면이라 생기던 각도별 잘림이
        /// 원리적으로 안 보이게 됩니다. 가까우면 지평선에 단면이 그어집니다.
        ///
        /// 2000 이었던 것을 1450 으로 줄였습니다. 안개가 제대로 닫히게 고친 뒤로는
        /// 그 너머가 어차피 안개뿐이라, 더 그리는 것은 순손해입니다.
        /// </summary>
        ...
        ///
        /// 시타델은 710 m, 대기권을 뚫는 기둥은 780 m 라 파클립(약 482 m) 밖으로
        /// 나갑니다. 그런데 <b>파클립은 구가 아니라 평면</b>이라 잘리는 깊이가
        /// <c>d·cos(각) + h·sin(각)</c> 으로 <b>시선의 상하 각도에 따라 변합니다</b> —
        /// 고개를 들면 꼭대기가 잘려 나가고 내리면 돌아옵니다.
        ///
        /// ⚠ <b>예전에는 안개가 그 자리를 가려 줬습니다.</b> 안개가 방사 거리로
        /// 완전히 닫히면 평면 깊이는 언제나 방사 거리 이하이므로 잘리는 자리가
        /// 반드시 안개 안이었습니다. <b>포그를 걷어낸 지금 그 보장은 없습니다.</b>
        /// 사거리 1450 m 가 세계(1100 x 1200 m)보다 넉넉해서 안 보일 뿐이고,
        /// 세계가 커지면 지평선에 잘린 단면이 그어집니다.
        ///
        /// <b>사다리는 건드리지 않습니다.</b> 씬의 파클립은 <c>SetViewBase</c> 의
        /// 기준이고 거기서 나무·지형 거리가 파생되므로, 그 값을 올리면 세계가
        /// 통째로 달라집니다. 대신 카메라의 파클립만 늘리고
