...
public class TimeDebugOverlay : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        rig = FindAnyObjectByType<WeatherRig>();
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(slowerKey)) SetSpeedIndex(speedIndex - 1);
        if (Input.GetKeyDown(fasterKey)) SetSpeedIndex(speedIndex + 1);
        if (Input.GetKeyDown(pauseKey)) time.paused = !time.paused;
    }
